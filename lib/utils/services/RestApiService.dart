...
class RestApiService {
  ...
  static Future<http.Response> get(String path, [Map<String, dynamic> queryParams = const {}]) async {
    final url = Uri.https(ApiPaths.base, path, queryParams);
    print('url is $url');
    return retry(() => http.get(url, headers: AppConstants.apiHeaders).timeout(Duration(seconds: 4)),
  ...
  static Future<http.Response> put(String path,
    ...
    print(url);
    print(requestBody);
    return retry(() => http.put(url, headers: AppConstants.apiHeaders, body: requestBody).timeout(Duration(seconds: 4)),
        retryIf: (e) => e is SocketException || e is TimeoutException, maxAttempts: 4);
  }
}
