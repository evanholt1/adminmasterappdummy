...
class VerifyOtp extends StatefulWidget {
  ...
  String? countryCode, title;

  VerifyOtp({Key? key, required this.mobileNumber, this.countryCode, this.title})
...
class _MobileOTPState extends State<VerifyOtp> with TickerProviderStateMixin {
      ...
      _firebaseAuth.signInWithCredential(_authCredential).then((UserCredential value) async {
        if (value.user != null) {
          ...
          if (widget.title == SEND_OTP_TITLE) {
            ...
            Future.delayed(Duration(seconds: 2)).then((_) {
              Navigator.pushReplacement(
                  context, MaterialPageRoute(builder: (context) => SetPass(mobileNumber: mobile!)));
            });
          }
        } else {
          setSnackbar(OTPERROR);
  ...
  mobText() {
    return Padding(
      padding: EdgeInsets.only(bottom: 10.0, left: 20.0, right: 20.0, top: 10.0),
      child: Center(
        child: Text("+$countrycode-$mobile",
            style: Theme.of(context).textTheme.subtitle1!.copyWith(color: fontColor, fontWeight: FontWeight.normal)),
      ),
    );
  }
